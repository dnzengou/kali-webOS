...
Kali-webOS v2.0.0 ships in **ten independent formats** from one source tree. Pick the one that fits.
...
| Tauri APK | Mobile, native FS access | `npm run tauri:android:build` | Rust + JDK 17 + Android SDK + NDK r26 |
| Bubblewrap APK | Play Store, lightweight | `npm run apk:twa` | JDK 17 + Android SDK + keystore |
| Docker image | Self-hosters, homelab, ARM SBCs | `npm run docker:build` | Docker + buildx |
| NPM SDK | Integrators (CoT engine only) | `npm run sdk:build` | — |
...
| **CLI (npx)** | Shell scripts, CI pipelines | `npm run cli:build` | Node 18 |

> **Evolve channels (extension / vscode / obsidian / cli)** all reuse the same
...
### 3. Android APK (Bubblewrap, light)
```bash
# one-time:
...
### 5. Use the AetherClaw SDK in another project
```bash
...
```bash
cd extension && npm i && npm run zip
# → extension/aetherclaw-extension-2.0.0.zip
# Load: chrome://extensions → Developer mode → Load unpacked → extension/dist/
```

### 7. VS Code extension
```bash
cd vscode-extension && npm i && npm run build && npm run package
...
### 8. Obsidian plugin
```bash
cd obsidian-plugin && npm i && npm run zip
# → obsidian-plugin/obsidian-aetherclaw-2.0.0.zip
# Unzip into <vault>/.obsidian/plugins/aetherclaw/ → enable in Settings → Community plugins
```

### 9. CLI (npx / global install)
```bash
cd cli && npm i && npm run build
node cli/dist/cli.js security "recon kali-webos.io"
# or after publish:
npx @aetherclaw/cli -a optimizer "audit ./src"
echo "deploy v2.0.0" | aetherclaw -a deployer --json
```

### 10. One-tag release (everything at once)
```bash
...
#   • SDK + CLI to npm (with --provenance)
#   • Browser extension .zip
#   • VS Code .vsix
#   • Obsidian plugin .zip
#   → 14+ artifacts attached to a single GitHub Release
```

### Build all evolve channels locally
```bash
npm run evolve:build   # sdk → ext → vscode → obsidian → cli
...
| Rust 1.77+ | Tauri desktop + mobile | `rustup` |
| Android SDK + NDK r26 | Tauri Android + Bubblewrap | Android Studio |
...
| "I want it on my phone, from Play Store" | Bubblewrap TWA (200 KB shell) |
| "I want it on my phone, with native APIs" | Tauri Android (12 MB) |
...
| "I want to chain it in shell scripts" | `npx @aetherclaw/cli` (JSON mode) |
